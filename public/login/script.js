...
if (document.getElementById('registerForm')) {
    document.getElementById('registerForm').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (errorDiv) {
                errorDiv.innerText = 'Could not connect to the database. Error: ' + err.message;
                errorDiv.style.display = 'block';
            } else {
                alert('Could not connect to the database. Error: ' + err.message);
            }
        } finally {
            if (btn) {
...
if (document.getElementById('loginForm')) {
    ...
    if (analyzeBtn) {
        analyzeBtn.addEventListener('click', async () => {
            const strengthInput = document.getElementById('strength-input').value.trim();

            if (!strengthInput) {
                alert('Please enter your strength or interest.');
                return;
            }
            ...
            try {
                ...
                const isSignedUp = users.some(user => user.email === localStorage.getItem('userEmail'));
